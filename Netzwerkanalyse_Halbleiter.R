# Netzwerkanalyse Handelsdaten Halbleiter
...
setwd("~/Desktop/Bachelorarbeit/R-Analyse/Halbleiter") #Desktop/Bachelorarbeit/R-Analyse/Halbleiter

# Daten einlesen:
###

trade_raw_data <- read.table("Halbleiter_Daten_China_Alle.csv", # öffne das .csv file,
...
updatedTrade_raw_data_origin <- aggregate(.~origin, trade_raw_data[c(1:320),], sum) # Zeilennummer an Herkunftsland Zeilennunmmer anpassen 
updatedTrade_raw_data_destination <- aggregate(.~destination, trade_raw_data[c(321:917),], sum) # Zeilennummer an Zielland Zeilennummer anpassen 
...
# ANPASSEN! Daten von "China" (Zeile 20) und "China, Hong Kong SAR" (Zeile 21) (Zeilen 20 and 21 in originalem Datensatz) zu einem werden lassen:
###

updatedTrade_raw_data_origin[17, 1] <- "China"
updatedTrade_raw_data_origin[18, 1] <- "China"# in Zeile 21 Spalte 1 soll "China" reingeschrieben werden
levels(updatedTrade_raw_data_origin[,1]) <- c(levels(updatedTrade_raw_data_origin[,1]), "Taiwan")
# ACHTUNG! Nummer der Faktorstufe "Other Asia, nes" ermitteln:
updatedTrade_raw_data_origin[,1] # 54 im Moment -> 54 in das folgende schreiben
updatedTrade_raw_data_origin[54, 1] <- "Taiwan"
# Kontrollieren, ob "Other Asia, nes" nun "Taiwan" ist
updatedTrade_raw_data_origin[54,1]
updatedTrade_raw_data_origin <- aggregate(.~origin, updatedTrade_raw_data_origin, sum)


# ANPASSEN! Falls "Zielland Data frame" auch "China" und "China, Hong Kong SAR" und "China, Macao SAR" hat, dasselbe tun (dann die beiden folgenden Zeilen zu einem Befehl umwandeln mit shift+command+c:
###

updatedTrade_raw_data_destination[27, 1] <- "China"
updatedTrade_raw_data_destination[28,1] <- "China"
updatedTrade_raw_data_destination[29,1] <- "China"
levels(updatedTrade_raw_data_destination[,1]) <- c(levels(updatedTrade_raw_data_destination[,1]), "Taiwan")
# ACHTUNG! Nummer der Faktorstufe "Other Asia, nes" ermitteln:
updatedTrade_raw_data_destination[,1] # 96 im Moment -> 96 in das folgende schreiben
updatedTrade_raw_data_destination[96, 1] <- "Taiwan"
updatedTrade_raw_data_destination <- aggregate(.~destination, updatedTrade_raw_data_destination, sum)

# Zeilen von China in origin und destination löschen
updatedTrade_raw_data_origin <- subset(updatedTrade_raw_data_origin, origin != "China")
updatedTrade_raw_data_destination <- subset(updatedTrade_raw_data_destination, destination != "China")
...
# -> Nun haben wir zwei data frames:einen für das gleiche Herkunfsland (außer China), einen für dasselbe Zielland (außer China)
...
year = c(rep(2021, each=55)) # Hinweis: bei anderen Daten Anzahl der Zeilen anpassen
destination = c(rep("China", each=55)) # Hinweis: bei anderen Daten Anzahl der Zeilen anpassen
updatedTrade_raw_data_origin <- cbind(updatedTrade_raw_data_origin, year) # mit 61 Zeilen
updatedTrade_raw_data_origin <- cbind(updatedTrade_raw_data_origin, destination) # mit 61 Zeilen
...
year = c(rep(2021, each=113)) # Hinweis: bei anderen Daten Anzahl der Zeilen anpassen
origin = c(rep("China", each=113)) # Hinweis: bei anderen Daten Anzahl der Zeilen anpassen
updatedTrade_raw_data_destination <- cbind(updatedTrade_raw_data_destination, year) # mit 105 Zeilen
updatedTrade_raw_data_destination <- cbind(updatedTrade_raw_data_destination, origin) # mit 105 Zeilen
...
# Zeilen löschen von "Eswatini", weil es die nicht im Geocode gab
###
...
final_trade_data <- subset(final_trade_data, origin != "Eswatini")
final_trade_data <- subset(final_trade_data, destination != "Eswatini")

# final_trade_data <- subset(final_trade_data, origin != "Congo")
# final_trade_data <- subset(final_trade_data, destination != "Congo")
...
get_graph <- function(final_trade_data) { 
  ...
  countriesgeocodes <- read_csv("geocode_palestine_from_nature_paper_Czechia_Rep. of Korea_USA_North Macedonia_Rep. of Moldova_United Rep. of Tanzania_Viet Nam_EU-27_changed.csv", na = "null")
...
# ANPASSEN!
###########
legend_1 <- "Globales Handelsnetzwerk Halbleiter"
